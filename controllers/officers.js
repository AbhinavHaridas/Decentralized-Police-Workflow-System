...
const insertOfficer = (req, res) => {
  ...
  connection.query(
    "INSERT INTO officers (full_name, dob, email, password, contact, city, zonal_code, rank, department_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [
      full_name,
      dob,
      email,
    ...
    (err, result) => {
      if (err) res.json(responseFormatter(500, err, "Error"));
...
const loginOfficer = (req, res) => {
  ...
  let query = "";
  if (emailRegex.test(user_input))
    query = "SELECT * FROM officers WHERE email = ?";
    ...
    if (err) res.status(500).json(responseFormatter(500, err, "Error"));
      ...
      if (result.length > 0) {
        ...
        if (match) {
          ...
          res.json(responseFormatter(200, officer, "Success"));
        } else {
          // If password does not match, return an error
          res.json(responseFormatter(400, false, "Wrong password"));
        }
      } else {
        // If User was not found, return an error
        res.json(responseFormatter(400, false, "User not found"));
      }
    }
  });
};
